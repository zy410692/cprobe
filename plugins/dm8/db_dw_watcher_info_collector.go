...
func NewDbDwWatcherInfoCollector(db *sql.DB, config *Config) MetricCollector {
...
func (c *DbDwWatcherInfoCollector) Collect(ch chan<- prometheus.Metric) {
	...
	rows, err := c.db.QueryContext(ctx, QueryDwWatcherInfoSql)
	if err != nil {
		handleDbQueryErrorWithSQL(QueryDwWatcherInfoSql, err)
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&info.DwMode, &info.DwStatus, &info.AutoRestart, &info.DwStatusToNum); err != nil {
			logger.Errorf("Error scanning row error:%s ", err)
			continue
		}
	...
	if err := rows.Err(); err != nil {
		logger.Errorf("Error with rows error:%s ", err)
	}
