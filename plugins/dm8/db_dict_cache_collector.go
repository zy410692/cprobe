...
func NewDbDictCacheCollector(db *sql.DB, config *Config) MetricCollector {
	...
	c.fieldCheckOnce.Do(func() {
		...
		for _, field := range fields {
			...
			if err := c.db.QueryRowContext(ctx, query).Scan(&count); err != nil {
				logger.Warnf("Failed to check V$DB_CACHE field SQL:%s: %v", query, err)
				continue
			}
			if count > 0 {
				c.availableFields = append(c.availableFields, field)
				//logger.Infof("V$DB_CACHE field %s exists", field)
			}
		}

		//logger.Infof("V$DB_CACHE available fields: %v", c.availableFields)
	})
...
func (c *DbDictCacheCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if len(availableFields) == 0 {
		logger.Warnf(" No dictionary cache fields available in V$DB_CACHE")
		return
	}
	...
	if err != nil {
		if err == sql.ErrNoRows {
			logger.Infof("No dictionary cache data available,SQL: %s", query)
		} else {
			handleDbQueryErrorWithSQL(query, err)
		}
		return
	}
